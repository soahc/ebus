...
public class AuthenticatedSession extends AuthenticatedWebSession {
	...
	public IDAOProvider getDAOProviderService() {
		...
		final BundleContext ctx = FrameworkUtil.getBundle(AuthenticatedSession.class).getBundleContext();
		final ServiceReference serviceReference = ctx.getServiceReference(IDAOProvider.class.getName());
	...
	public boolean authenticate(String username, String password) {
		final SystemUser user = getDAOProviderService().getSystemUserDAO().readSystemUser(username, password);
